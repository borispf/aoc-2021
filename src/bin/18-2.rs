...
fn solve(i: &[u8]) -> u64 {
    let ss = parse(i);
    let mut max_mag = 0;
    for (i, j) in (0..ss.len()).cartesian_product(0..ss.len()) {
        if i == j { continue; }
        let mut tsi = vec![];
        let mut tsj = vec![];
        ss[i].to_list(&mut tsi);
        ss[j].to_list(&mut tsj);
        let mag = S::from_list(&add(&tsi, &tsj)).1.magnitude();
        max_mag = std::cmp::max(max_mag, mag);
    }
    max_mag
}
